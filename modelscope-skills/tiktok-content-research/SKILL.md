---
name: "tiktok-content-research"
description: "当用户需要做 TikTok 内容研究、账号内容观察、竞品内容对比、趋势判断或素材整理时使用。面向内容运营、品牌调研和创作者。"
source_client: "socialdatax-skills"
source_platform: "modelscope"
source_skill: "tiktok-content-research"
...
当用户需要做 TikTok 内容研究、账号内容观察、竞品内容对比、趋势判断或素材整理时使用。面向内容运营、品牌调研和创作者。
...
npx -y socialdatax-skills@latest tiktok search \
  ...
  --source-platform modelscope --source-skill tiktok-content-research
```

## 参数说明

搜索：
- 说明：TikTok `search --keyword <text>`：使用 `tiktok search` 时必填；使用用户真实意图，去掉多余空格，并保持关键词聚焦。
- 可选：`--max-items <n>`：收集到 N 条搜索结果后停止。
- 说明：TikTok `--content-type <all|video|image>`：可选内容类型筛选；不传表示包含全部内容类型。
- 说明：TikTok `--pages <n>`：从当前起点继续获取并合并 N 页搜索结果；如果返回了 `next_page_token`，可继续续页。

通用：
- 可选：`--pretty`：只影响输出格式，不改变实际请求结果。
- 说明：TikTok `--page-token <next_page_token>`：这是不透明的分页 token；第一页不要传。继续同一条搜索链路时，只能原样传回完整返回的 `next_page_token`。
- 可选：`--source-client socialdatax-skills --source-platform modelscope --source-skill tiktok-content-research`：这是当前 Agent Skill 的来源标记；按本 Skill 示例执行时保持这些值不变。
