---
name: "kuaishou-trend-insights"
description: "当用户需要做快手趋势洞察、快手趋势分析、关键词观察、内容方向判断、趋势线索归纳或营销灵感整理时使用。面向内容运营、品牌调研和创作者。"
source_client: "socialdatax-skills"
source_platform: "modelscope"
source_skill: "kuaishou-trend-insights"
...
当用户需要做快手趋势洞察、快手趋势分析、关键词观察、内容方向判断、趋势线索归纳或营销灵感整理时使用。面向内容运营、品牌调研和创作者。
...
npx -y socialdatax-skills@latest kuaishou hot-search \
  ...
  --source-skill kuaishou-trend-insights

npx -y socialdatax-skills@latest kuaishou search \
  --keyword "<keyword>" --pretty --source-client socialdatax-skills \
  --source-platform modelscope --source-skill kuaishou-trend-insights
```

## 参数说明

热榜：
- 说明：Kuaishou `hot-search`：无必填参数。

搜索：
- 说明：Kuaishou `search --keyword <text>`：使用 `kuaishou search` 时必填；使用用户真实意图，去掉多余空格，并保持关键词聚焦。
- 说明：Kuaishou `--pages <n>`：从当前起点继续获取并合并 N 页搜索结果；如果返回了 `next_page_token`，可继续续页。
- 可选：`--max-items <n>`：收集到 N 条搜索结果后停止。

通用：
- 可选：`--pretty`：只影响输出格式，不改变实际请求结果。
- 说明：Kuaishou `--page-token <next_page_token>`：这是不透明的分页 token；第一页不要传。继续同一条搜索链路时，只能原样传回完整返回的 `next_page_token`，不能截断、改写、脱敏、重建，或用省略号替换中间内容。
- 可选：`--source-client socialdatax-skills --source-platform modelscope --source-skill kuaishou-trend-insights`：这是当前 Agent Skill 的来源标记；按本 Skill 示例执行时保持这些值不变。

如果用户要看当前快手热榜，使用 `kuaishou hot-search`；这个命令不需要 `--keyword`。
...
快手搜索翻页：
- 只有 `next_page_token` 非空时才继续翻页。
- 同一条搜索翻页链路下，只能把完整返回的 `next_page_token` 原样作为 `page_token` 传回；不要截断、改写、脱敏、重建，也不要用省略号替换中间内容。
...
快手搜索结果需要可追溯时，保留 `photo_id`、`share_url`、作者事实和可见互动数据。
...
如果当前 Agent 已可直接调用 MCP 工具，调用 `kuaishou_get_hot_search_list` 时不要传关键词参数。

不要只因为某一页 `items` 为空就判断没有更多结果。
快手 MCP 调用 `kuaishou_search_videos`：
...
调用 `kuaishou_search_videos` 时不要传 `page`；第一页也不要传 `page_token`。
快手只有在 `next_page_token` 非空时才继续翻页；同一个关键词链路下，把完整返回的 `next_page_token` 原样作为 `page_token` 传回。
