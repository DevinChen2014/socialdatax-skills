...
npx -y socialdatax-skills@latest douyin search \
  ...
  --source-platform modelscope --source-skill douyin-competitor-research
```
...
- 可选：`--source-client socialdatax-skills --source-platform modelscope --source-skill douyin-competitor-research`：这是当前 Agent Skill 的来源标记；按本 Skill 示例执行时保持这些值不变。
