...
description: "当用户需要做 B站评论分析、B站评论洞察、B站用户反馈分析、B站需求挖掘、痛点总结、购买顾虑整理、FAQ 提炼、口碑分析、评论回复观察或内容讨论复盘时使用。基于用户提供的 B站视频、专栏、动态链接或对应内容 ID 下的评论结果，面向内容运营、产品调研、品牌调研和创作者。"
source_client: "socialdatax-skills"
source_platform: "modelscope"
source_skill: "bilibili-comment-insights"
...
当用户需要做 B站评论分析、B站评论洞察、B站用户反馈分析、B站需求挖掘、痛点总结、购买顾虑整理、FAQ 提炼、口碑分析、评论回复观察或内容讨论复盘时使用。基于用户提供的 B站视频、专栏、动态链接或对应内容 ID 下的评论结果，面向内容运营、产品调研、品牌调研和创作者。

## 快速开始

- 先给出当前 skill 支持的输入：视频、专栏、动态链接或内容 ID。
...
- 说明：Bilibili 回复 `--comment-object-id <comment_object_id>`、`--comment-object-type <comment_object_type>` 和 `--comment-id <comment_id>`：使用一级评论结果里返回的回复定位参数。

通用：
- 可选：`--pretty`：只影响输出格式，不改变实际请求结果。
- 可选：`--source-client socialdatax-skills --source-platform modelscope --source-skill bilibili-comment-insights`：这是当前 Agent Skill 的来源标记；按本 Skill 示例执行时保持这些值不变。
...
- `bilibili_get_content_comments_by_id`：已知 Bilibili `content_id` 时使用；可选 `sort_type` 支持 `hot` 或 `time_descending`。
- `bilibili_get_content_comments_by_url`：用于 Bilibili 视频、文章、动态、短链或分享文本；可选 `sort_type` 支持 `hot` 或 `time_descending`。
- `bilibili_get_content_comment_replies_by_comment_id`：已知 `comment_object_id`、`comment_object_type` 和一级评论 ID 时使用。
