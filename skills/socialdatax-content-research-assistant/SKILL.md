...
- XHS numeric `page` is only for XHS search; Douyin, Kuaishou, Weibo, and WeChat Channels search use `page_token` only and do not accept `page`. Omit `page_token` on the first token-paginated search request, and continue only with the complete returned `next_page_token` from the same chain.
- `--page-token <next_page_token>`: use for Douyin/Kuaishou/Weibo/WeChat Channels search continuation and for token-paginated comments, replies, creator posts, creator notes, creator videos, and creator series. Keep the original topic, content item, or creator target stable. Do not modify, truncate, redact, mask, omit, normalize, rebuild, generate, or replace the middle with ellipses.
...
Use numeric `page` only for `xhs_search_notes`. Do not pass `page` to `douyin_search_videos`, `kuaishou_search_videos`, `weibo_search_posts`, or `wechat_search_videos`; omit `page_token` on the first request and pass only the complete returned `next_page_token` when continuing the same chain.
