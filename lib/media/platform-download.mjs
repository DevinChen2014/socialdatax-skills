...
const PLATFORM_MEDIA_DOWNLOAD_CONFIGS = {
  xhs: {
    platform: "xhs",
    displayName: "XHS",
...
const MEDIA_DOWNLOAD_MAX_ATTEMPTS = 10;
const MEDIA_DOWNLOAD_RETRY_DELAY_MS = 250;
const MEDIA_RETRYABLE_HTTP_STATUSES = new Set([
  408,
  409,
  425,
  429,
...
const MEDIA_EXPIRED_HTTP_STATUSES = new Set([403, 404, 410]);
const MEDIA_CONTENT_TYPE_EXTENSIONS = new Map([
  ["image/jpeg", "jpg"],
  ["image/jpg", "jpg"],
  ["image/png", "png"],
  ["image/webp", "webp"],
...
const MEDIA_URL_FORMAT_EXTENSIONS = new Map([
  ["jpg", "jpg"],
  ["jpeg", "jpg"],
  ["png", "png"],
  ["webp", "webp"],
...
export async function downloadPlatformMediaFromUrl(
  ...
  const config = resolvePlatformMediaDownloadConfig(platform);
  const parsedUrl = parseHttpUrl(url, config);
  const target = await resolvePlatformMediaTarget(parsedUrl, options, now, config);
  const existingOutput = await stat(target.outputPath).catch(ignoreMissing);
  if (existingOutput?.isDirectory()) {
    throw new Error(`--output must be a file path for ${config.platform} download-media.`);
  }
  if (existingOutput) {
  ...
  const proxyDispatcher = createMediaDownloadProxyDispatcher(parsedUrl, options, {
    config,
    env,
  });

  try {
    return await downloadWithPartResume(parsedUrl, target, {
      config,
      fetchMedia,
      downloadTimeoutMs,
      maxDownloadBytes,
    ...
    if (proxyDispatcher) {
      await proxyDispatcher.close().catch(ignoreError);
    }
  }
}

async function resolvePlatformMediaTarget(parsedUrl, options = {}, now, config) {
  if (options.output && options.outputDir) {
    throw new Error(
      `Use only one of --output or --output-dir for ${config.platform} download-media.`
    );
  }
  if (options.output) {
    const outputPath = resolve(expandHome(options.output));
    await ensureParentDirectory(outputPath, "--output parent path", config);
    return {
      outputPath,
  ...
  if (!options.outputDir) {
    throw new Error(`Missing --output or --output-dir for ${config.platform} download-media.`);
  }
  ...
  if (existingOutputDir && !existingOutputDir.isDirectory()) {
    throw new Error(`--output-dir must be a directory for ${config.platform} download-media.`);
  }
  if (!existingOutputDir) {
    await ensureParentDirectory(outputDir, "--output-dir parent path", config);
    await mkdir(outputDir, { recursive: true });
  }

  const fileTarget = inferMediaFileTarget(parsedUrl, now, config);
...
async function downloadWithPartResume(
  ...
  for (let attempt = 1; attempt <= MEDIA_DOWNLOAD_MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (error instanceof PlatformExpiredMediaLinkError) {
        throw error;
      }
      lastError = error;
  ...
  if (isAbortLikeError(lastError)) {
    throw new Error(formatPlatformMediaTimeoutMessage(config));
  }
  if (isNetworkLikeError(lastError)) {
    throw new Error(formatPlatformMediaNetworkFailureMessage(config));
  }
  throw new Error(`${config.displayName} media download failed before completion.`);
}
...
async function downloadOnce(
  ...
  const headers = normalizePlatformDownloadHeaders(config);
  ...
  if (MEDIA_EXPIRED_HTTP_STATUSES.has(response.status)) {
    await cancelResponseBody(response);
    throw new PlatformExpiredMediaLinkError(config);
  }
  ...
  if (!response.ok) {
    await cancelResponseBody(response);
    if (MEDIA_RETRYABLE_HTTP_STATUSES.has(response.status)) {
      throw new MediaRetryableDownloadError();
    }
    throw new Error(
      `${config.displayName} media download failed with HTTP ${response.status}.`
    );
  }
  ...
  await finalizePartFile(partPath, outputPath, { config, linkFile, copyFile });
  return {
    outputPath,
    outputBytes,
...
async function finalizePartFile(
  ...
  try {
    await ensureParentDirectory(
      outputPath,
      `${config.platform} download-media output parent path`,
      config
    );
    try {
    ...
    if (isExistingOutputPathError(error)) {
      throw await buildExistingOutputPathError(outputPath, config);
    }
    throw error;
  }
...
function resolvePlatformMediaDownloadConfig(platform) {
  const config = PLATFORM_MEDIA_DOWNLOAD_CONFIGS[platform];
  if (!config) {
    throw new Error(`Unsupported media download platform: ${platform}`);
  }
  return config;
}

function parseHttpUrl(value, config) {
  let parsed;
  try {
    parsed = new URL(String(value || ""));
  } catch {
    throw new Error(`${config.displayName} media URL must be an HTTP URL.`);
  }
...
function createMediaDownloadProxyDispatcher(parsedUrl, options, { config, env }) {
  const proxyUrl = resolveMediaDownloadProxyUrl(parsedUrl, options, {
    config,
    env,
  });
...
function resolveMediaDownloadProxyUrl(parsedUrl, options = {}, { config, env }) {
  if (options.proxy !== undefined) {
    return normalizeMediaDownloadProxyUrl(options.proxy, "--proxy", config);
  }
  const envName = MEDIA_DOWNLOAD_PROXY_ENV_BY_PROTOCOL[parsedUrl.protocol]?.find(
    (name) => normalizeOptionalString(env?.[name])
  );
  if (!envName) {
    return "";
  }
  return normalizeMediaDownloadProxyUrl(env[envName], envName, config);
}

function normalizeMediaDownloadProxyUrl(value, displayName, config) {
  ...
  try {
    ...
    throw new Error(
      `${displayName} must be an HTTP or HTTPS proxy URL for ${config.platform} download-media.`
    );
  }
  if (!["http:", "https:"].includes(parsed.protocol) || !parsed.hostname) {
    throw new Error(
...
function normalizePlatformDownloadHeaders(config) {
  return {
...
function inferMediaFileTarget(parsedUrl, now, config) {
  const fallbackStem = `${config.platform}-media-${formatTimestamp(now())}`;
...
async function ensureParentDirectory(outputPath, displayName, config) {
  const parent = dirname(outputPath);
  const existingParent = await stat(parent).catch(ignoreMissing);
  if (existingParent && !existingParent.isDirectory()) {
    throw new Error(`${displayName} must be a directory for ${config.platform} download-media.`);
  }
  if (!existingParent) {
...
function buildSkippedExistingResult(platform, url, outputPath, outputBytes) {
  return {
    platform,
    action: "download-media",
...
function shouldRetryDownloadError(error) {
  return (
    error instanceof MediaRetryableDownloadError ||
...
function formatPlatformMediaTimeoutMessage(config) {
  if (config.platform === "x") {
    return [
      "X / Twitter media download timed out.",
      "X media is served from overseas CDN domains such as pbs.twimg.com and video.twimg.com;",
      "make the CLI process use a proxy with --proxy \"http://127.0.0.1:7890\" or HTTP_PROXY / HTTPS_PROXY / ALL_PROXY, then retry.",
    ].join(" ");
  }
  return `${config.displayName} media download timed out.`;
}

function formatPlatformMediaNetworkFailureMessage(config) {
  if (config.platform === "x") {
    return [
      "X / Twitter media download could not reach the media CDN.",
      "X media is served from overseas CDN domains such as pbs.twimg.com and video.twimg.com;",
      "make the CLI process use a proxy with --proxy \"http://127.0.0.1:7890\" or HTTP_PROXY / HTTPS_PROXY / ALL_PROXY, then retry.",
    ].join(" ");
  }
  return `${config.displayName} media download failed before completion.`;
}
...
function mediaExtensionFromContentType(contentType) {
  return MEDIA_CONTENT_TYPE_EXTENSIONS.get(contentType) || "";
}
...
function inferUrlFormatExtension(parsedUrl) {
  ...
  if (matched) {
    return MEDIA_URL_FORMAT_EXTENSIONS.get(sanitizeFileExtension(matched[1])) || "";
  }
  return (
    MEDIA_URL_FORMAT_EXTENSIONS.get(
      sanitizeFileExtension(parsedUrl.searchParams.get("format"))
    ) || ""
  );
}
...
async function buildExistingOutputPathError(outputPath, config) {
  const existingOutput = await stat(outputPath).catch(ignoreMissing);
  if (existingOutput?.isDirectory()) {
    return new Error(`--output must be a file path for ${config.platform} download-media.`);
  }
  return new Error(`${config.displayName} media download output file already exists.`);
}
...
class MediaRetryableDownloadError extends Error {}

class PlatformExpiredMediaLinkError extends Error {
  constructor(config) {
    super(
      `${config.displayName} media link is unavailable or expired. Get a fresh media URL from the ${config.displayName} detail result and retry.`
    );
  }
}
